...
def fetch(ticker: str, period: str = "2y", market: str = "se") -> dict:
    ...
    with connect(market) as c:
        for idx, r in hist.iterrows():
            ...
            # Yahoo returnerar ibland NaN för enskilda dagar (t.ex. runt
            # en handelsstopp eller ett datahål hos leverantören). NaN
            # klarar float()-konverteringen ovan utan att kasta fel
            # (float('nan') är giltig Python), men SQLite vägrar sedan
            # skriva den till en NOT NULL-kolumn — det är det som
            # kraschade hela körningen tidigare. Explicit NaN-koll här
            # (x == x är False bara för NaN) fångar det innan det når
            # databasen, så EN dålig rad för ETT bolag inte stoppar
            # datahämtningen för alla resterande bolag i universum.
            ...
            c.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (ticker, idx.date().isoformat(), open_, high, low, close, volume),
            )
            rows += 1

    if skipped:
        logger.warning("%s: hoppade över %d rad(er) med saknad/ogiltig data",
                       ticker, skipped)
